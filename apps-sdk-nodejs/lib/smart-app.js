...
module.exports = class SmartApp {
	...
	constructor(options = {}) {
		...
		if (options.publicKey) {
			this._signature.setPublicKey(options.publicKey)
		}
	...
	keyUrl(url) {
		this._keyUrl = url
		this._signature.publicKeyUrl = url
		return this
	}
	...
	async handleHttpCallback(request, response) {
		...
		} else if (await this._signature.isAuthorized(request)) {
			this._handleCallback(request.body, responders.httpResponder(response, this._log))
		} else {

...
module.exports = class ApiApp {
	...
	constructor(options = {}) {
		...
		this._signature = new Signature(this._log, this._keyUrl)
	...
	/**
	 * Manually set the SmartThings KEY URL
	 * @param {String} url
	...
	isAuthorized(request) {
		return this._signature.isAuthorized(request)
	}
	 ...
	 * @param {*} response
	 */
	async handleEventCallback(request, response) {
	...
	async _handleCallback(body, responder) {
		...
		switch (messageType) {
			case 'EVENT': {
				...
				for (const event of eventData.events) {
					switch (event.eventType) {
						case 'INSTALLED_APP_LIFECYCLE_EVENT': {
							...
							switch (installedAppLifecycleEvent.lifecycle) {
								case 'DELETE':
									...
									if (this._contextStore) {
										this._contextStore.delete(installedAppId)
									}
						...
						default: {
							this._log.warn(`Unhandled event of type ${event.eventType} ${JSON.stringify(event, null, 2)}`)
						}
					}
				}
			...
			case 'CONFIRMATION':
				try {
					await axios.get(body.confirmationData.confirmationUrl)
					this._log.info(`App ${body.confirmationData.appId} callback confirmed`)
					responder.respond({statusCode: 200, eventData: {}})
				} catch (error) {
					this._log.error(error)
					responder.respond({statusCode: error.statusCode || 500, message: error.message})
				}

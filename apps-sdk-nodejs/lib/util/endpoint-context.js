...
const putSegmentedName = Symbol('private')
const contextStore = Symbol('private')

module.exports = class EndpointContext {
	constructor(app, evt, apiMutex) {
		this.event = evt
		this[contextStore] = app._contextStore
	...
	async updateState(name, value) {
		...
		this[putSegmentedName](name, value)
	}

	saveState() {
		if (this.state) {
			return this[contextStore].update(this.installedAppId, {state: this.state})
		}
	}
